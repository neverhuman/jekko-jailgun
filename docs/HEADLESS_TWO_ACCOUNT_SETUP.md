...
For Jekko split-family automation, keep a local env file at
`~/.jekko/jailgun/browser-profiles.env` with the account ids, CDP endpoints,
profile pool, primary profile/state/download directories, and Chrome
executable. Start from `config/browser-profiles.env.example`; the populated
file must stay local-only with `0600` permissions.
